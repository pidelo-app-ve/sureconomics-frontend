/**
 * Detector de caras `pico.js`, incorporado al proyecto.
 *
 * Copia literal de https://github.com/tehnokv/picojs (licencia MIT, ver abajo), con
 * tres cambios y ninguno de fondo: se declaran dos variables que el original dejaba
 * implicitas (`pico` e `idx`) y el modulo exporta en vez de dejar un global.
 *
 * **Por que copiado y no como dependencia.** El paquete de npm arranca con
 * `pico = {}`, una asignacion a una variable sin declarar. Eso funciona en un
 * `<script>` suelto y revienta en modo estricto, que es como se ejecuta todo lo que
 * pasa por el empaquetador: `ReferenceError: pico is not defined`, comprobado. Como
 * son seis kilobytes y la licencia lo permite, sale mas barato traerlo que envolver
 * un paquete roto.
 *
 * El clasificador entrenado -- `public/facefinder.bin`, del repositorio original de
 * Nenad Markus -- va aparte porque son 234 KB de datos, no de codigo.
 */
